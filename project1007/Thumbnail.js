...
class Thumbnail{
    constructor(container, width, height, bd, src, x){
        ...
        this.img.addEventListener("click", function(){
            //alert("내 경로는"+this.src);
            //getDetail(this.src, '제목');
            //이벤트 구현시 사용되는 익명함수 내에서 this를 사용하게 되면,
            //객체의 인스턴스를 가리키게 되는 것이 아니라 익명함수의 {}를 가리키게 된다
        
            //현재 객체가 배열의 몇번째인지 알아맞히기!
            var index=thumbArray.indexOf(me);
            console.log("index: ", index);
            //alert("저는 thumbArray의 "+index+"번째에 있으니, marvel 배열의 "+index+"번쨰를 이용하겠습니다");
            getDetail(index);

            //포인터의 targetX값은 나의 좌표와 일치
